...
class GradeCase(TransactionCase):
    def setUp(self):
        super(GradeCase, self).setUp()
        self.course1 = self.env.ref("event_training.course_odoo_mixed")
        self.course2 = self.course1.copy()
        self.event = self.env.ref("event_training.event_odoo_mixed")
        self.reg = self.env.ref("event_training.student_thomas_passot")

    def test_grade_min_above_max(self):
        """Block setting minimum grade above maximum."""
        with self.assertRaises(ex.GradeLimitOverflowError):
            self.course1.grade_min = self.course1.grade_max + 0.01

    def test_grade_max_below_min(self):
        """Block setting maximum grade below minimum."""
        with self.assertRaises(ex.GradeLimitOverflowError):
            self.course1.grade_max = self.course1.grade_min - 0.01

    def test_grade_pass_above_max(self):
        """Block setting passing grade above maximum."""
        with self.assertRaises(ex.GradePassingOverflowError):
            self.course1.grade_pass = self.course1.grade_max + 0.01

    def test_grade_pass_below_min(self):
        """Block setting passing grade below minimum."""
        with self.assertRaises(ex.GradePassingOverflowError):
            self.course1.grade_pass = self.course1.grade_min - 0.01

    def test_grade_pass_equal_limits(self):
        """Passing grade can be equal to maximum or minimum."""
        self.course1.grade_pass = self.course1.grade_min
        self.course1.grade_pass = self.course1.grade_max
    ...
    def test_grade_equals_limits(self):
        ...
        """
        self.reg.grade = self.course1.grade_min
        self.reg.grade = self.course1.grade_pass
        self.reg.grade = self.course1.grade_max

    def test_grade_below_limit(self):
        """Registration gets grade below allowed limit."""
        with self.assertRaises(ex.StudentGradeOutsideLimitsError):
            self.reg.grade = -0.01

    def test_grade_above_limit(self):
        """Registration gets grade above allowed limit."""
        with self.assertRaises(ex.StudentGradeOutsideLimitsError):
            self.reg.grade = 10.01
